import { GetlistofDisctrictByStateId, GetListOfState, GetListOfVDCByDistrictId } from "../Constants/Url";
...
export const getlistofDisctrictByStateIdApi = async (data, successCallback) => {
    try {
        const response = await fetch(`${GetlistofDisctrictByStateId}?stateId=${data.stateId}`);
        if (response?.status === 200) {
...
export const getListOfVDCByDistrictIdApi = async (data, successCallback) => {
    try {
        const response = await fetch(`${GetListOfVDCByDistrictId}?districtId=${data.districtId}`);
        if (response?.status === 200) {
